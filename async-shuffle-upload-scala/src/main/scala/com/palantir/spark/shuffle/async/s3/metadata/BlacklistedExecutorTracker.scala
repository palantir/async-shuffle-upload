...
final class BlacklistedExecutorTracker {

  private val blacklistedExecutors = Sets.newConcurrentHashSet[BlockManagerId]()

  def blacklistExecutor(blockManagerId: BlockManagerId): Unit = {
    blacklistedExecutors.add(blockManagerId)
  }

  def getBlacklistedExecutors: java.util.Set[BlockManagerId] = {
    ImmutableSet.copyOf[BlockManagerId](blacklistedExecutors)
  }
}
